...
- addCard(card: ICard): void - добавляет одну карточку в начало массива и вызывает событие изменение массива (брокер событий)
...
- а так-же сеттеры и геттеры для сохранения и получения данных из полей класса(поля с подчеркиваниями -\_cards,\_preview - данные получаются с помощью геттеров, а устанавливаются с помощью сеттеровв)
...
- open(handleSubmit: Function): void - расширение родительского метода, принимает обработчик, который передается при инициации события подтверждения. (при открытии мо передаем обрабочик для того действия,которое должно подтверждаться)
...
- submitButton: HTMLButtonElement - Кнопка подтверждения
- \_form: HTMLFormElement - элемент формы
- formName: string - значение атрибута name формы, с какой конкретно формой мы взаимодействуем? - идентифицируем\ для какой формы будем генерировать событие
...
- setError(data: { field: string, value: string - какие данные введены, validInformation: string -какой текст ошибки надо отобразить}): void - \ ..принимает объект с данными для отображения или сокрытия текстов ошибок под полями ввода
...
#### Класс Card - управляем разметкой карточки
...
- `card:selected` - изменение открываемой в модальном окне картинки карточки(карточка выбрана - см. \_preview: у CardsData )
...
- `card:like` - изменение состояния лайка на карточке
  Примечание: используется атрибут name, чтобы понять, какая именно форма сгенерировала нам событие
  Ввод:
...
- `remove-card:submit` - событие, генерируемое при нажатии "Да" в форме подтверждения
  Валидация -проверка всей формы а не как при инпуте - только проверка валидации в одном поле
